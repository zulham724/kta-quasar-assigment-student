...
<div>
    <q-card>
            ...
            <q-item class="q-pa-none">
                <q-item-section avatar>
                    <q-avatar size="2.5rem">
                        <q-skeleton type="QAvatar" size="100%" style="position:absolute" />
                        <q-img style="z-index:1" no-default-spinner :src="`${Setting.storageUrl}/${post.author_id.avatar}`"></q-img>
                    </q-avatar>
                </q-item-section>
                <q-item-section>
                    <q-item-label>
        ...
        <q-card-section class="q-py-xs">
            <div class="text-caption" style="overflow-wrap:break-word; white-space:pre-line">{{ post.body.length>100?post.body.substring(0,100)+'...':post.body }}</div>

            <div v-if="post.body.length > 100" @click="$router.push(`/post/comment/${post.id}`)" class="text-caption text-grey">
                Readmore
            </div>
        ...
        <q-card-section class="q-pt-xs q-pb-sm">
            <div class="row full-width" style="color:rgba(0, 0, 0, 0.54)">
                <div class="col-3">
                    <div class="row">
                        <span class="material-icons" :style="`font-size:20px;${post.liked_count ? 'color:red' : null}`" clickable @click="post.liked_count ? dislike() : like()">
                            {{post.liked_count ? 'favorite' : 'favorite_border'}}
                        </span>
                        <span @click="$router.push(`/post/like/${post.id}`)">{{ post.likes_count }} Suka</span>
                    </div>
                </div>
                <div class="col-4" clickable @click="$router.push(`/post/comment/${post.id}`)">
                ...
                <div class="col-5 text-right">
                    <span v-if="true" class="material-icons" style="font-size:20px" clickable @click="$q.notify('Dalam konstruksi')">
                        turned_in
                    </span>
                </div>
            </div>
...
export default {
    ...
    computed: {
        ...mapState(["Setting", "Auth", 'MediaPost'])
    },
    data() {
        return {
            slide: 0,
    ...
    created() {
        console.log(this.MediaPost.posts.data)
    },
    methods: {
        sendNotif() {
            const payload = {
